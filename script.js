...
(function () {
  ...
  function choose(state) {
    ...
    document.dispatchEvent(new CustomEvent('lindjanar:consent', { detail: state }));
    if (state === 'granted') {
...
(function () {
  document.addEventListener('click', function (e) {
    ...
    if (window.posthog) window.posthog.capture('contact_click', { method: isTel ? 'phone' : 'email' });
  }, true);
})();
...
// ============================================================
// PostHog (EU cloud) – heatmaps, session replay, funnels
//   Loads NOTHING until the cookie banner has been accepted; a
//   declined or unanswered banner means no script, no cookie, no
//   recording. Every form field is masked in recordings so phone
//   numbers and e-mails never leave the visitor's browser.
//   The phc_ key is a public write-only token (safe in page source).
// ============================================================
(function () {
  var KEY = 'phc_nexMh7HBHzEaPjtruRpRLLkEcsNsu4ZG3qYc5ZTxKLBv';
  var loaded = false;

  function load() {
    if (loaded) return;
    loaded = true;
    /* official snippet, pointed at the EU ingestion host */
    ...
    window.posthog.init(KEY, {
      ...
      session_recording: {
        maskAllInputs: true,
        maskTextSelector: 'a[href^="tel:"], a[href^="mailto:"]'
      }
    });
  }
  ...
  document.addEventListener('lindjanar:consent', function (ev) {
    if (ev.detail === 'granted') load();
  });
})();
