  ...
  function steps() {
    ...
    if (S.object === 'korter' || S.object === 'eramaja') {
      ...
      if (S.size) {
        ...
        list.push({ id: 'need', q: T('Millist paketti soovid?', 'Which package would you like?'),
          hint: (S.size === '150+' ? T('Üle 200 m² ja villad hindame eraldi. ', 'Over 200 m² and villas are quoted separately. ') : '') + T('Allpool näha kõiki valikuid, alates maakleri koostööst kuni tavaliste kinnisvarafotodeni.', 'All the options are below, from working with an agent to standard property photos.'),
          ...
          opts: [
            { v: 'maakler', b: T('Maaklerit', 'An agent'), s: T('teeb kõik algusest lõpuni – hinnastamine, kuulutus, näitamised, tehing', 'handles everything start to finish – pricing, listing, viewings, the deal'), why: T('Kui kaldud maakleri poole, aitame valida õige – Richard helistab.', 'Leaning towards an agent? We help you pick the right one – Richard calls.'), price: '2\u00a0000–5\u00a0000 €' },
            { v: 'koik', badge: T('+403 % rohkem päringuid', '+403 % more enquiries'), b: T('Kõik koos', 'Everything'), s: T('fotod + droonifotod + Listing Reel', 'photos + drone photos + Listing Reel'), sum: eur(t.foto) + ' + ' + eur(P.droon) + ' + ' + eur(P.reel), price: T('alates ', 'from ') + eur(t.foto + P.droon + P.reel) },
            { v: 'foto-droon', badge: T('+94 % rohkem vaatamisi', '+94 % more views'), b: T('Fotod + droonifotod', 'Photos + drone photos'), s: S.object === 'korter' ? T('hoone ja ümbrus õhust', 'the building and surroundings from the air') : T('maja ja krunt õhust', 'the house and plot from the air'), why: droneWhy, sum: eur(t.foto) + ' + ' + eur(P.droon), price: eur(t.foto + P.droon) },
            { v: 'foto', b: T('Fotod', 'Photos'), s: L(t.fotos) + T(' · käes 48-tunniga', ' · delivered in 48 h'), price: eur(t.foto) },
